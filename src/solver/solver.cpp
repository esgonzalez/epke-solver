...
Solver::Solver(
    ...
    : time(time), gen_time(gen_time), pow_norm(pow_norm), rho_imp(rho_imp),
      ...
      delta_t(time.size()), power(time.size()), H(time.size()),
...
const double
Solver::computeOmega(const PrecIndex& k, const TimeIndex& n) const {
...
const double
Solver::computeZetaHat(const PrecIndex& k, const TimeIndex& n) const {
  const auto beta_eff_k = &precursors.at(k)->delayedFraction();
  const auto lambda_k = &precursors.at(k)->decayConstant();
...
void Solver::computeA1B1(
  ...
  a1 = gamma_d * pow_norm.at(n) / E(lambda_h_n, delta_t_n) *
...
const double Solver::computeABC(const TimeIndex& n, const double theta) const {
...
const bool Solver::acceptTransformation(const TimeIndex& n) const {
